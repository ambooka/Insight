...
import com.msah.insight.utils.ViewUtils;
...
public class StepperView extends FrameLayout {
     ...
     */
    @Nullable
    private StepperView mPrevItemView, mNextItemView;
    ...
    public StepperView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
         this.setOnClickListener(new OnClickListener() {
             ...
             public void onClick(View v) {
                 Toast.makeText(context,  ""+getIndex(), Toast.LENGTH_SHORT).show();
        ...
        setDoneIcon(getResources().getDrawable(R.drawable.check));
        setActivatedColor(getResources().getColor(R.color.colorAccent));
        setAnimationEnabled(true);
        setLineColor(getResources().getColor(android.R.color.black));
    ...
    private void prepareViews(@NonNull Context context) {
        ...
        response = inflateView.findViewById(R.id.stepper_summary);
     ...
     */
    public synchronized void setState(@StepperView.State int state) {
        ...
        if (mErrorText != null) {
            ...
            mSummaryColorAnimator = ViewUtils.createArgbAnimator(
                    response, "textColor", response.getCurrentTextColor(), mErrorColor);
            ...
            if (mSummaryColorAnimator != null) mSummaryColorAnimator.cancel();
            mSummaryColorAnimator = ViewUtils.createArgbAnimator(
                    response, "textColor", response.getCurrentTextColor(), mLineColor);
            mSummaryColorAnimator.setDuration(mAnimationDuration);
            mSummaryColorAnimator.start();
       ...
       // response.setVisibility(state != STATE_SELECTED && !TextUtils.isEmpty(mSummary) ? View.VISIBLE : View.GONE);
    ...
    public void setErrorText(@Nullable CharSequence errorText) {
        ...
        response.setText(mErrorText != null ? mErrorText : mSummary);
        setState(mState);
    }
     ...
     */
    public void bindSteppers(@Nullable StepperView prevItem, @Nullable StepperView nextItem) {
        if (prevItem != null) {
     ...
     */
    public static void bindSteppers(@NonNull StepperView... items) {
    ...
    public Parcelable onSaveInstanceState() {
        StepperView.ItemViewState state = new StepperView.ItemViewState(super.onSaveInstanceState());
        state.title = mTitle;
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof StepperView.ItemViewState) {
            StepperView.ItemViewState viewState = (StepperView.ItemViewState) state;
    ...
    protected static class ItemViewState extends BaseSavedState {
        ...
        public static final Creator<StepperView.ItemViewState> CREATOR =
                new Creator<StepperView.ItemViewState>() {
                    @Override
                    public StepperView.ItemViewState createFromParcel(Parcel source) {
                        return new StepperView.ItemViewState(source);
                    }

                    @Override
                    public StepperView.ItemViewState[] newArray(int size) {
                        return new StepperView.ItemViewState[size];
                    }
                };
